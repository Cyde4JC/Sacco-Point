...
export default function AppGlTransfer({ onClose, refresh }) {
...
<>
    <Card sx={{ mb: 1, padding: 2 }}>
      ...
      <CardContent>
        <Form methods={methods} onSubmit={onSubmit}>
          <Box gap={3} display="flex" sx={{ mt: 2 }} flexDirection="column">
            ...
            <Stack direction="row" spacing={2}>
              <Field.Text name="receiver_account_number" label="Debit Account Number" InputLabelProps={{ shrink: true }} />
              <Field.Text name="amount" label="Debit Amount" InputLabelProps={{ shrink: true }} />
              <Field.Text name="reason" label="Debit Reason" InputLabelProps={{ shrink: true }} />
            </Stack>
          </Box>
          <CardActions>
            <LoadingButton
      ...
      <Dialog open={openConfirmDialog} onClose={handleCloseConfirmDialog}>
              <DialogTitle>
                  <h3>Confirm the details before proceeding with the Debit.</h3>
                  <Divider sx={{ backgroundColor: '#800080' }} />
              </DialogTitle>
            <DialogContent>  
                <p style={{ fontSize: '14px' }}>
                  <strong>Debit Account Number:</strong> {formData?.receiver_account_number}
                </p>
                <p style={{ fontSize: '14px' }}>
                  <strong>Debit Amount:</strong> {fCurrency(formData?.amount)}
                </p>
                <p style={{ fontSize: '14px' }}>
                  <strong>Debit Reason:</strong> {formData?.reason}
                </p>
                <Divider sx={{ backgroundColor: '#800080' }} />        
            </DialogContent>
            <DialogActions>
